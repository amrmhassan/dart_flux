...
class FluxMemoryAuthCache implements AuthCacheInterface {
  // Lock objects for synchronization (for future thread-safety)
  ...
  void _notifyEviction(String cacheType, String key, EvictionReason reason) {
    if (_evictionController.hasListener) {
      _evictionController.add(
        EvictionEvent(cacheType: cacheType, key: key, reason: reason),
      );
    }
  }
  ...
  void _manageCacheSize<T>({
    ...
    for (var i = 0; i < entriesToRemove; i++) {
      if (queue.isNotEmpty) {
        ...
        _notifyEviction(
          getCacheTypeForItem<T>(),
          oldestKey,
          EvictionReason.sizeLimitReached,
        );
      }
    }
  }
  ...
  T _synchronized<T>(T Function() fn) {
    // In a truly concurrent environment, we would use a lock here
    // but for simplicity, we're just calling the function directly
    // since Dart is single-threaded per isolate
    return fn();
  }
